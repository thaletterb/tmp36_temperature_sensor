...
uint16_t sample_adc_channel(uint8_t);
...
// Measurements are gathered in time periods.
typedef struct { // time period type
  long temp_accum;     // accumulated temperature measurements
  int  temp_min;       // minimum temperature measured
  int  temp_max;       // maximum temperature measured
  int  photo_sun;      // full sun readings
  int  photo_shade;    // shade (anything less then full sun) readings
  int  reading_cnt;     // reading counter
  int  reading_cnt_max; // max reading_cnt for this period.
} time_period_t;
